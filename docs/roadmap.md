...
- 다나와 PC견적 스킬 출시 (샵다나와 공개 PC견적 화면 요청 기반 부품 검색·가격·호환성 체크)
- 번개장터 검색 스킬 출시
- 중고차 가격 조회 스킬 출시
- 한국어 맞춤법 검사 스킬 출시
- 한국어 글자 수 세기 스킬 출시
...
- **업데이트(2026-05)**: `danawa-pc-estimate` 스킬이 샵다나와 PC견적 공개 화면 요청을 사용해 부품 검색, 가격 추출, 조건 필터, 호환성 체크를 shipped 상태로 제공한다.
- 남은 확장 후보: 일반 다나와 가격비교 전체 카테고리, 판매처별 배송비/쿠폰 비교, 상세 가격 추이
